...
public sealed class AddListEmployeeAsync : BaseEmployeeManagerMock
{
    private readonly RequestAddListEmployeeDto _dto;
    private readonly int _userId;

    public AddListEmployeeAsync()
    {
        _dto = new RequestAddListEmployeeDto
        {
            WorkObject = new RequestGetWorkObjectDto { WorkObjectId = 1 },
            Employees = new List<BaseEmployeeDto>
            {
                new() { Firstname = "John", Surname = "Doe", Lastname = "Smith" }
            }
        };
        _userId = 1;
    }
    ...
    [Fact]
    public async Task AddEmployeeListAsync_ReturnsResponseBaseWithBadRequest_WhenWorkObjectNotFound()
    {
        // Arrange
        ...
        // Act
        var actualResult = await _employeeManager.AddEmployeeListAsync(_dto, _userId);
    ...
    [Fact]
    public async Task AddEmployeeListAsync_ReturnsResponseBaseWithOk_WhenWorkObjectFound()
    {
        // Arrange
        ...
        // Assert
        _empRepositoryMock.Verify(x => x.InsertRangeAsync(It.IsAny<IEnumerable<Employee>>()),Times.Once);
        Assert.Equal(expectedResult.IsSuccessful, actualResult.IsSuccessful);
    }
}